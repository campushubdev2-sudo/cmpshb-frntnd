...
const generateMockNotifications = (): MockNotification[] => {
  ...
  return [
    ...
    },
    {
      ...
      message: "This is a friendly reminder about an upcoming event. Please make sure to attend on time.",
      status: "sent",
    ...
    },
    {
      ...
      message: "There has been an important update regarding this event. Please check the event details for more information.",
      status: "failed",
...
export default function NotificationsPage() {
  ...
  const [notifications, setNotifications] = useState<MockNotification[]>(() => generateMockNotifications());
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = (error as { response?: { data?: { message?: string } } })?.response?.data?.message || "Failed to send notification";
      toast.error(message);
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDelete = async () => {
    try {
      ...
      const message = (error as { response?: { data?: { message?: string } } })?.response?.data?.message || "Failed to delete notification";
      toast.error(message);
    } finally {
      setDeleting(false);
    }
  ...
  const columns: ColumnDef<MockNotification>[] = [
    {
      ...
      cell: (row: MockNotification) => {
        const event = typeof row.eventId === "string" 
          ? getEventById(row.eventId) 
          : row.eventId;
        return event?.title || "—";
      },
    },
    {
      ...
      cell: (row: MockNotification) => {
        const user = typeof row.recipientId === "string" 
          ? getUserById(row.recipientId) 
          : row.recipientId;
    ...
    },
    {
      ...
      accessorKey: "sentAt",
      cell: (row: MockNotification) => 
        format(new Date(row.sentAt), "MMM dd, yyyy HH:mm"),
    },
    {
      header: "Actions",
      accessorKey: "actions",
